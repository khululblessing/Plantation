        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    yBill.BillCode = "OG-Book-Inv#" + new Random().Next(1, 9999);
                    yBill.Email = customer.Email;
                    yBill.Name = customer.FirstName;
                    yBill.RegNo = customer.CustomerID;
                    yBill.billID = new Random().Next(1, 999).ToString();
                    yBill.Status = "No funds are due at the moment!";
                    yBill.bookingtotal = 0;
                    yBill.TotalAmount = 0;
                    ...
                    orderBill.BillCode = "OG-Order-Inv#" + new Random().Next(1, 9999);
                    orderBill.Email = customer.Email;
                    orderBill.Name = customer.FirstName;
                    orderBill.RegNo = customer.CustomerID;
                    orderBill.billID = new Random().Next(1, 999).ToString();
                    orderBill.Status = "No funds are due at the moment!";
                    orderBill.OrderAmount = 0;
                    orderBill.TotalAmount = 0;

                    Db.OrderBills.Add(orderBill);
                    Db.SaveChanges();

                    Chatter chat = new Chatter();

                    chat.Email = customer.Email;
                    chat.Name = customer.FirstName;
                    chat.Surname = customer.LastName;


                    Db.Chatters.Add(chat);
                    Db.SaveChanges();
        ...
        public async Task<ActionResult> Assistant(RegisterViewModel model, HttpPostedFileBase img_upload)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    customer.ContactNo = model.ContactNo;
                    customer.Email = model.Email;
                    customer.AssistantImage = model.picture;
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                else if (user != null || (await UserManager.IsEmailConfirmedAsync(user.Id)))
                {
                    ...
                    MailMessage mm = new MailMessage("sandilemnyanduvincentmbandlwa@gmail.com", model.Email);
                    mm.Subject = "Rest your password now";
                    mm.Body = "To reset your password, kindly click this link " + callbackUrl + "\n" +
                               " Kind regards " + "\n" +
                               "Josiah's Multiplayer team";
                    mm.IsBodyHtml = false;
                    ...
                    NetworkCredential nc = new NetworkCredential("sandilemnyanduvincentmbandlwa@gmail.com", "bcssowotzqfzyvwp");
